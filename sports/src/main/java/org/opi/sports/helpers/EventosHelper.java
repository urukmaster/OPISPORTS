 ...
 *
 *Sprint 02 Descripción:Esta clase sirve de apoyo a la clase de EventosController
 *y contiene la lógica necesaria para serializar los eventos y poder mostrarlas en el front end
	...
	*//**
	...
	public List<CalendarioPOJO> calendarioSerializer(
		...
		for (EventoPOJO eventoView : listaEventosView) {
			...
			startTime = convertirFechaHora.parseDateTime(convertirFecha.format(eventoView.getFecha()) + " " + convertirHora.format(eventoView.getHora().getTime()));
			endTime = convertirFechaHora.parseDateTime(convertirFecha.format(eventoView.getFecha()) + " " + convertirHora.format(sumarHora.getTime()));
			
			
			eventos.setTitle("Cancha Sintetica #5");
			eventos.setStart(startTime);
			eventos.setEnd(endTime);
